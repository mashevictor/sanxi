...
export function warmDispatchCaches(dataDir: string): void {
  ...
  console.log(
    `[warm] 派单缓存: full-match ${full?.pairings?.length ?? 0} 条 · 后道 ${back?.dispatch?.stats?.matched ?? 0} · 前道 ${front?.dispatch?.stats?.matched ?? 0} · 金山32+15 ${jinshanPreset?.dispatch?.stats?.matched ?? '—'} · 公交 ${legs.size} 条`
  );
}
